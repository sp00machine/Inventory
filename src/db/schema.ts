...
import { pgTable, uuid, text, integer, boolean, type AnyPgColumn, uniqueIndex } from "drizzle-orm/pg-core";
...
export const locations_table = pgTable("locations", {
    ...
    parent_location_id: uuid("parent_location_id").references((): AnyPgColumn => locations_table.id as any, { onDelete: 'set null' })
});
...
export const asins_table = pgTable("asins", {
    asin: text("asin").primaryKey(), // Each ASIN code must be unique and is the primary key
    product_type_id: uuid("product_type_id").notNull().references(() => product_types_table.id),
...
export const inventory_items_table = pgTable("inventory_items", {
    id: uuid("id").primaryKey().defaultRandom(),
    product_type_id: uuid("product_type_id").notNull().references(() => product_types_table.id),
    asin: text("asin").notNull().references(() => asins_table.asin), // The specific ASIN barcode on this item
    ...
    location_id: uuid("location_id").references(() => locations_table.id, { onDelete: 'set null' }),
...
export const item_moves_table = pgTable("item_moves", {
    ...
    asin: text("asin").notNull().references(() => asins_table.asin),
    quantity: integer("quantity").notNull(),
    initial_location_id: uuid("initial_location_id").notNull().references(() => locations_table.id),
    new_location_id: uuid("new_location_id").notNull().references(() => locations_table.id),
