...
export const load: PageServerLoad = async () => {
  ...
  const storage_locations = locations.filter(
    (loc) => loc.id !== db.HOLDING_LOCATION_ID,
  );
...
export const actions: Actions = {
  ...
  default: async ({ request }) => {
    ...
    try {
      ...
      const condition_note = new_condition
        ? ` (condition changed to ${new_condition})`
        : "";
      const status_note = new_status
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Decant operation failed",
      };
    }
  },
};
