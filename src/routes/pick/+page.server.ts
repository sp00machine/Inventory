...
export const load: PageServerLoad = async () => {
  const [locations, asins, holding_location, inventory_items] =
    await Promise.all([
      db.getAllLocations(),
      db.getAllAsins(),
...
export const actions: Actions = {
  ...
  default: async ({ request }) => {
    ...
    const to_location_id =
      (formData.get("to_location_id") as string) || db.HOLDING_LOCATION_ID;
    ...
    try {
      ...
      const destination_name =
        to_location_id === db.HOLDING_LOCATION_ID
          ? "Holding Area"
          : to_location.name;
