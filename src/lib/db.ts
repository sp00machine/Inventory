...
import { inventory_items_table, locations_table, product_types_table, asins_table, item_moves_table } from '../db/schema';
...
export async function getAllInventoryItems() {
    const items = await db.select({
        ...
        .leftJoin(product_types_table, eq(inventory_items_table.product_type_id, product_types_table.id))
        .leftJoin(asins_table, eq(inventory_items_table.asin, asins_table.asin))
        .leftJoin(locations_table, eq(inventory_items_table.location_id, locations_table.id));

    return items;
}

// Adds a new inventory item, or merges it with an existing one if the same
export async function addOrMergeinventoryItem(newItem: typeof inventory_items_table.$inferInsert) {
    // try to find an existing item with the same product_type_id, asin, condition, status, and location_id
    const existingItem = await db.select().from(inventory_items_table).where(and(
    ...
    if (existingItem.length > 0) {
        ...
        const updatedQuantity = (existingItem[0].quantity || 0) + (newItem.quantity || 0);
        await db.update(inventory_items_table)
        ...
        const result = await db.insert(inventory_items_table).values(newItem).returning();
        return result[0];
    }
}
// =============== Product Types ===============
...
export async function getProductTypeById(id: string) {
    return await db.select().from(product_types_table).where(eq(product_types_table.id, id));
}

// Add a new product type
export async function addProductType(productType: typeof product_types_table.$inferInsert) {
    console.log('Inserting product type:', productType);
    const result = await db.insert(product_types_table).values(productType).returning();
...
export async function updateProductType(id: string, updates: Partial<typeof product_types_table.$inferInsert>) {
    await db.update(product_types_table)
        .set(updates)
...
export async function getAllAsins() {
    return await db.select({
        ...
        .from(asins_table)
        .leftJoin(product_types_table, eq(asins_table.product_type_id, product_types_table.id));
}
...
export async function getProductTypesWithAsins() {
    ...
    for (const product_type of product_types) {
        ...
        const total_quantity = inventory_stats.reduce((sum, item) => sum + (item.total_quantity || 0), 0);
        const unique_locations = [...new Set(inventory_stats.map(item => item.location_name).filter(Boolean))];
        ...
        for (const asin of asins) {
            const asin_inventory = await db.select({
                quantity: inventory_items_table.quantity,
                condition: inventory_items_table.condition,
                status: inventory_items_table.status,
                location_name: locations_table.name,
            ...
            const asin_total_quantity = asin_inventory.reduce((sum, item) => sum + (item.quantity || 0), 0);
            const asin_locations = [...new Set(asin_inventory.map(item => item.location_name).filter(Boolean))];
            ...
            asins_with_stats.push({
                ...
                stats: {
                    ...
                    condition_breakdown: asin_inventory.reduce((acc, item) => {
                        const condition = item.condition || 'unknown';
                        acc[condition] = (acc[condition] || 0) + (item.quantity || 0);
                        return acc;
                    }, {} as Record<string, number>),
                    status_breakdown: asin_inventory.reduce((acc, item) => {
        ...
        result.push({
            ...
            stats: {
                ...
                condition_breakdown: inventory_stats.reduce((acc, item) => {
                    const condition = item.condition || 'unknown';
                    acc[condition] = (acc[condition] || 0) + (item.total_quantity || 0);
                    return acc;
                }, {} as Record<string, number>),
                status_breakdown: inventory_stats.reduce((acc, item) => {
                    const status = item.status || 'unknown';
...
export async function getAsinsByProductType(product_type_id: string) {
    return await db.select().from(asins_table)
...
export async function getAsinByCode(asinCode: string) {
    const results = await db.select().from(asins_table)
...
export async function getAsinByAsin(asin: string) {
    const results = await db.select().from(asins_table).where(eq(asins_table.asin, asin));
    return results[0] || null;
}
// Add a new ASIN
...
// Add a new location
export async function addLocation(location: typeof locations_table.$inferInsert) {
...
export async function getLocationByName(locationName: string) {
    return await db.select().from(locations_table).where(eq(locations_table.name, locationName)).then(results => results[0] || null);
}

// Get a location by ID
export async function getLocationById(locationId: string) {
    return await db.select().from(locations_table).where(eq(locations_table.id, locationId)).then(results => results[0] || null);
}

// Get a location and its contents
export async function getLocationWithContents(locationId: string) {
    const location_promise = getLocationById(locationId);

    const contents_promise = await db.select({
        id: inventory_items_table.id,
        product_type_id: inventory_items_table.product_type_id,
        asin: inventory_items_table.asin,
        condition: inventory_items_table.condition,
    ...
    const [location, contents] = await Promise.all([location_promise, contents_promise]);
...
export async function recordItemMove(move: typeof item_moves_table.$inferInsert) {
...
export async function getMovesByAsin(asin: string) {
    return await db.select().from(item_moves_table)
...
export async function moveInventory(params: {
    ...
    const { asin, from_location_id, to_location_id, quantity, move_type, new_condition, new_status, user, notes } = params;

    // Find existing inventory items at the source location with this ASIN
    const source_items = await db.select().from(inventory_items_table)
        .where(and(
    ...
    const total_available = source_items.reduce((sum, item) => sum + (item.quantity || 0), 0);

    if (total_available < quantity) {
        throw new Error(`Insufficient quantity. Available: ${total_available}, Requested: ${quantity}`);
    }
    ...
    for (const source_item of source_items) {
        ...
        if (quantity_to_take === item_quantity) {
            ...
            await db.delete(inventory_items_table)
